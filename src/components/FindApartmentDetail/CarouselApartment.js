...
const CarouselApartment = ({images}) => {
  ...
  return (
    ...
    <div className='absolute top-[45%] z-20 py-5 px-1 bg-teal-600 rounded-lg left-0'>
        <button type='button' onClick={() => setCurrentImage(prevState => prevState === 0 ? prevState : prevState -  1)} className='text-white'>
    ...
    <div className="carousel-inner h-[500px] relative w-full overflow-hidden rounded-lg">
        <img className='h-[600px] w-full ' src={images[currentImage]}/>
    </div>
    <div className='absolute top-[45%] z-20 py-5 px-1 bg-teal-600 rounded-lg right-0'>
        <button type='button' onClick={() => setCurrentImage(prevState => prevState === images.length - 1 ? prevState : prevState + 1 )} className='text-white'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8">
